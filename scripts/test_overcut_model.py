...
df = df.dropna(subset=REQUIRED + ['overcut_success']).copy()
for col in FEATURES:
...
def mode_scenario():
    ...
    tire_age_delta    = tire_age - ca_tire_age        # positive = stay-out more worn
    ...
    row['ca_tire_age']     = float(ca_tire_age)
    row['tire_age_delta']  = float(tire_age_delta)
    ...
    print(f"    Tire ages:        yours={tire_age}L  car_ahead={ca_tire_age}L  "
          f"(delta = {tire_age_delta:+d} laps, positive = you more worn)")
