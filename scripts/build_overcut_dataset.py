...
# ── Helpers (shared with undercut pipeline) ──────────────────────────────────

def build_gap_timeseries(session):
    """
    For every (driver, lap) return gap_ahead, gap_behind,
    car_ahead_driver, and car_behind_driver using LapStartTime differences.
    """
    ...
    laps['car_behind_driver']    = grp['Driver'].shift(-1)   # needed for overcut
    ...
    return laps[['Driver', 'LapNumber',
                 'gap_ahead', 'gap_behind',
                 'car_ahead_driver', 'car_behind_driver']].copy()
